  ...
  class Grammar
    ...
    def vobject_grammar
      ...
      contentline = seq(linegroup._?, C::NAME_VCAL, params._?, ":",
        ...
        errors << Paramcheck.paramcheck(strict, key, params.empty? ? {} : params[0], @ctx)
      ...
      calpropname = /CALSCALE/i.r | /METHOD/i.r | /PRODID/i.r | /VERSION/i.r |
        ...
        C::XNAME_VCAL | C::IANATOKEN
      calprop  = seq(calpropname, params._? << ":".r, C::VALUE, /(\r|\n|\r\n)/) do |key, params, value, _|
        key = key.upcase.gsub(/-/,"_").to_sym
        val, errors1 = Typegrammars.typematch(strict, key, params[0], :CALENDAR, value, @ctx)
        errors << errors1
        hash = { key => { value: val } }
        errors << Paramcheck.paramcheck(strict, key, params.empty? ? {} : params[0], @ctx)
        hash[key][:params] = params[0] unless params.empty?
