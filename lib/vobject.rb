...
module Vobject
 ...
 class << self
  ...
  def vobjectGrammar
    ...
    params 	= seq(';'.r >> param) {|p| p[0]} | seq(';'.r >> param, lazy{params} ) {|p, ps|
			p.merge(ps)
		}
    ...
    vobject 	= seq(beginLine, versionLine, rest).fail('vobject') { |(beginline, versionline, rest)|
			comp = beginline[:BEGIN].to_sym
	            	hash = { comp => versionline.merge( rest ) }
		        hash
		}
    vobject.eof 
  end 
